...
async def nlr(ctx: commands.Context):	
  ...
  try:	
        ...
        embed.add_field(name="> Progress", value=f'`{str(datetime.timedelta(seconds=j["now_playing"]["elapsed"]))} {progress_bar} {str(datetime.timedelta(seconds=j["now_playing"]["duration"]))}`'.replace(f'{str(datetime.timedelta(seconds=j["now_playing"]["elapsed"]))} {progress_bar} 0:00:00', f"{j['live']['streamer_name']} is Live").replace("[ is Live]", "[ERROR]"), inline=False)
